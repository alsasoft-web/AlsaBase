...
async function runTests() {
  ...
  const filterExpr = client.filter(
    "status = {:status} && age > {:age} && title ~ {:title}",
    {
      status: "active",
      age: 25,
      title: 'test "quote"',
  ...
  const builtUrl = client.buildUrl("/api/test", {
    page: 1,
    search: "hello world",
  });
  console.log("   Built URL:", builtUrl);
  if (
    !builtUrl.includes("page=1") ||
    !builtUrl.includes("search=hello+world")
  ) {
  ...
  const recordFileUrl = client.files.getUrl(
    { collectionName: "posts", id: "rec_123" },
    "cover.png",
  );
  console.log("   Record File URL:", recordFileUrl);
  if (
    !recordFileUrl.includes("/api/static-files/file/posts/rec_123/cover.png")
  ) {
    throw new Error("Record file URL generation failed");
  }

  const publicUrl = client.files.getPublicUrl("test/index.html");
  console.log("   Public Asset URL:", publicUrl);
  if (!publicUrl.includes("/test/index.html")) {
  ...
  try {
    ...
    console.warn(
      "   Server health check note (server may not be running locally on 8090):",
      err.message,
    );
  }
  ...
  try {
    ...
    if (err instanceof ClientResponseError) {
      console.log(
        `   Caught expected ClientResponseError with status code: ${err.status}`,
      );
    } else {
  ...
  const unsubscribe = client.authStore.onChange((token, model) => {
    listenerCalled = true;
    console.log(
      `   AuthStore change detected. Token: "${token ? token.substring(0, 10) + "..." : ""}", Model ID: ${(model as any)?.id}`,
    );
  });

  client.authStore.save("mock.jwt.token", {
    id: "user_test_1",
    email: "test@alsabase.local",
  });
  ...
  if (
    (client.authStore.token as string) !== "" ||
  ...
  if (
    typeof client.getSchema !== "function" ||
    typeof client.getTableSchema !== "function"
  ) {
    throw new Error("client.getSchema or client.getTableSchema method missing");
  }
  if (
    typeof client.collections.getSchema !== "function" ||
    typeof client.collections.getTableSchema !== "function"
  ) {
    throw new Error("client.collections.getSchema method missing");
  }
  const postsService = client.collection("posts");
  if (
    typeof postsService.getSchema !== "function" ||
    typeof postsService.schema !== "function" ||
    typeof postsService.getTableSchema !== "function"
  ) {
    throw new Error(
      "postsService.getSchema or postsService.schema method missing",
    );
  }
  // Test 7: Realtime Socket.IO subscription interface
  console.log("7. Testing Realtime Socket.IO Service interface...");
  if (
    typeof client.realtime.subscribe !== "function" ||
    typeof client.realtime.unsubscribe !== "function" ||
    typeof client.realtime.publish !== "function"
  ) {
    throw new Error("client.realtime methods missing");
  }
  const unsubRealtime = await client.realtime.subscribe(
    "posts",
    (_event) => {},
  );
  if (typeof unsubRealtime !== "function") {
    throw new Error(
      "client.realtime.subscribe did not return unsubscribe function",
    );
  }
  unsubRealtime();
  ...
  console.log(
    "All AlsaBase SDK unit and integration checks executed successfully!",
  );
}
